...
class DCGAN(object):
    ...
    Arguments:
        ...
        image_height: output images's height
        image_width: output images's width
    ...
    def discriminator(self, x, reuse, trainable=True):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            for i in range(len(self.discriminator_filters_list)):
                ...
                output = tf.layers.conv2d(output, filters, kernel_size, strides=strides,
                                          padding="SAME", trainable=trainable)
                output = tf.layers.batch_normalization(output, training=trainable)
...
dcgan = DCGAN(dataset_floder='./faces', images_save_floder='./generated_faces')
dcgan.train()