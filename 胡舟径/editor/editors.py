...
def modify(number,direct,coeffs):
        # 在这儿选择生成器
    tflib.init_tf()
    with open('model/generator_yellow.pkl', "rb") as f:
        generator_network, discriminator_network, Gs_network = pickle.load(f)
    ...
    # 在这儿选择人物的潜码，注意要与生成器相匹配。潜码来自生成目录下有个generate_codes文件夹里的txt文件。
    face_latent = read_feature('../result/generate_code/'+str(number).zfill(4)+'.txt')
    stack_latents = np.stack(face_latent for _ in range(1))
    face_dlatent = Gs_network.components.mapping.run(stack_latents, None)
    ...
    '''
        ...
        exposure.npy - 调整光照
    '''
    dict = {"年龄":"age.npy","笑容":"smile.npy","性别":"gender.npy","眼睛的闭合程度":"eyes_open.npy","脸的宽度":"width.npy"}
    direction = np.load('latent_directions/'+dict[direct])
